...
import com.bornfire.xbrl.entities.BRF.DEMO_ENTITY;
...
public class DEMOReportService {
	...
	private static final Logger logger = LoggerFactory.getLogger(DEMOReportService.class);
	...
	public String preCheck(String reportid, String fromdate, String todate) {
			...
			try {
				...
				Long dtlcnt = (Long) hs.createQuery("select count(*) from DEMO_ENTITY a where a.report_date=?1")
						.setParameter(1, dt9).getSingleResult();

				if (dtlcnt > 0) {
					logger.info("Getting No of records in Mod table :" + reportid);
					Long modcnt = (Long) hs.createQuery("select count(*) from DEMO_ENTITY a").getSingleResult();
					if (modcnt > 0) {
						msg = "success";
					}
				} else {
	...
	public ModelAndView getDemoView(String reportId, String fromdate, String todate, String currency, String dtltype,
			Pageable pageable) {
			...
			List<DEMO_ENTITY> T1rep = new ArrayList<DEMO_ENTITY>();
			// Query<Object[]> qr;

			List<DEMO_ENTITY> T1Master = new ArrayList<DEMO_ENTITY>();
			...
			//	T1rep = t1CurProdServiceRepo.getT1CurProdServices(d1);
				...
				T1Master = hs.createQuery(" from  DEMO_ENTITY a where a.report_date = ?1 ", DEMO_ENTITY.class)
